...
public class CrawlerTaskDao {
	...
	public static CrawlerTask getTaskByProjectAndType(String puuid, String type) throws SQLException{
		List<CrawlerTask> crawlerTasks = DAOUtils.getResult(CrawlerTask.class, 
				"select * from crawlerTask where projectUuid = ? AND resourceType = ?",
				puuid,type);
		if (crawlerTasks.size() == 0) return null;
		return crawlerTasks.get(0);
	}

	public static List<CrawlerTask> getTaskByProject(String puuid) throws SQLException{
		List<CrawlerTask> crawlerTasks = DAOUtils.getResult(CrawlerTask.class, 
				"select * from crawlerTask where projectUuid = ?",
				puuid);
		return crawlerTasks;
	}
